...
STEP 03. 현재 User.plist 파일에는 아무런 내용도 없기 때문에 프로퍼티 툴 역시 Root 노드를 제외하면 텅 비어 있는 상태이다. 유일한 항목인 Root는 편집 툴에서 하위 항목을 추가하기 위해 제공되는 가상의 최상위 노드로 단지 개념상으로만 존재하기 때문에 실제 XML 코드에는 포함되지 않는다.
